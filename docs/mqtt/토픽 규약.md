...
> **타입별 payload 의 최종 기준은 [`시나리오 계약 v1.md`](<./시나리오 계약 v1.md>)** 이며, 두 문서가 충돌하면 시나리오 계약 v1을 따릅니다.
> 처음 오신 분은 [`MQTT 계약 쉽게 이해하기.md`](<./MQTT 계약 쉽게 이해하기.md>)(비유로 푼 설명판)를 먼저 읽으면 빠릅니다.
...
시나리오 상태와 전체 흐름은 [`../scenario/귀가 환영 시나리오.md`](<../scenario/귀가 환영 시나리오.md>)를 참고합니다.
...
flowchart LR
    subgraph 살아있음["실제로 동작하는 타입"]
        A1[DOOR_OPENED]:::alive --> A2[현관 인사 시나리오]:::alive
        B1[AMBIENT_ENVIRONMENT_OBSERVED]:::alive --> B2[관측 기록 + 온습도 안부]:::alive
        C1[WAKE_WORD_DETECTED]:::alive --> C2[보미야 호출]:::alive
        D1["NAVIGATION_RESULT (v1)"]:::alive --> D2[시나리오 진행]:::alive
        E1[FOLLOW_RESULT]:::alive --> E2[호출·산책·귀가 후속]:::alive
        F1[CONVERSATION_STARTED / ENDED]:::alive --> F2[대화 상태 전이]:::alive
        G1[WALK_REQUESTED]:::alive --> G2[산책 시나리오]:::alive
        H1[MOTION_DETECTED]:::alive --> H2[방향 판정 버퍼]:::alive
        I1[DOOR_CLOSED]:::alive --> I2[로그만]:::alive
        J1[REST_STATE_CHANGED]:::alive --> J2[휴식 관찰·모드 토글]:::alive
        K1[NAVIGATION_STATUS]:::alive --> K2[로그만]:::alive
    end
    subgraph 죽음["검사는 통과하지만 아무 일도 일어나지 않음"]
        X1[PRESENCE_DETECTED]:::nohandler
        X2[ONBOARDING_ANSWER_CAPTURED]:::nohandler
        X3[SPEAK_RESULT]:::nohandler
        X4[CANCEL_RESULT]:::nohandler
        X1 & X2 & X3 & X4 --> XN["핸들러 없음 → 무시 로그"]:::nohandler
    end
    subgraph 거부["보내면 통째로 폐기됨"]
        Y1[HEARTBEAT]:::rejected
        Y2["허용 목록 밖 type"]:::rejected
        Y3["QoS ≠ 1"]:::rejected
        Y4["retain = true"]:::rejected
        Y1 & Y2 & Y3 & Y4 --> YN["오류 응답 없음"]:::rejected
    end

    classDef alive fill:#adf0c7,stroke:#087429
    classDef nohandler fill:#fff6b6,stroke:#af7e02
    classDef rejected fill:#ffc6c6,stroke:#bd0909
```
...
Backend가 받는 IoT 이벤트 타입은 다섯입니다. 타입별 payload 규정은 [`시나리오 계약 v1.md`](<./시나리오 계약 v1.md>) §4.1을 따릅니다.
...
`intent`는 `WELLNESS_CHECK｜MEDICATION_REMINDER｜HOMECOMING_GREETING` 셋뿐입니다. `triggerContext`는 계약 예시보다 필드가 많을 수 있으므로 엄격 파싱하지 마십시오. 자세한 형식은 [`시나리오 계약 v1.md`](<./시나리오 계약 v1.md>) §6.1을 따릅니다.
...
`WAKE_WORD_DETECTED`·`WALK_REQUESTED`·`CONVERSATION_STARTED`·`CONVERSATION_ENDED`의 payload 규정은 [`시나리오 계약 v1.md`](<./시나리오 계약 v1.md>) §4.1·§6.2·§6.3을 따릅니다. 아래에는 이 문서가 책임지는 상관관계 배치만 적습니다.
...
종료 후 분기는 시나리오마다 다릅니다 — 현관 인사는 `reasonCode`에 따라 `NAVIGATE(DEFAULT)` 복귀와 `FOLLOW_START` 추종으로 갈립니다([`시나리오 계약 v1.md`](<./시나리오 계약 v1.md>) §8.5).
