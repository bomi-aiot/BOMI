...
## 현관 이벤트 타입 (S15P11E102-226)

| `type` | 센서 | 뜻 |
| --- | --- | --- |
| `DOOR_OPENED` | SONOFF SNZB-04P | 문 접점이 열렸다 |
| `MOTION_DETECTED` | SONOFF SNZB-03P | 현관에서 움직임이 있었다 |
| `HEARTBEAT` | 라즈베리파이 | 현관 노드가 살아 있다 |
...
상관 판정 시간 창은 실측값이며 `bomi.entrance.correlation-window` 로 조정합니다. 이 판정을 서버에 둔 이유가 그것입니다 — 튜닝에 펌웨어나 로봇 배포가 필요해서는 안 됩니다.

`MOTION_DETECTED` 는 **아직 배포된 펌웨어가 발행하지 않습니다.** 서버는 준비돼 있고, 그 토픽이 조용한 동안에는 통과가 판정되지 않을 뿐 아무것도 깨지지 않습니다.

### 시각

`occurredAt` 은 규약대로 타임존을 포함한 ISO 8601 이지만, **백엔드는 순서 판정에 도착 시각을 씁니다.** 배터리 백업 RTC 가 없는 라즈베리파이는 시계가 몇 년 어긋난 채로 부팅할 수 있고, 그 값으로 순서를 매기면 귀가가 외출로 뒤집힙니다. 원본 시각은 기록만 하며, 어긋남이 크면 로봇이 경고를 남깁니다.
