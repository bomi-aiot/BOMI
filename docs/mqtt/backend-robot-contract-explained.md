...
> 이 문서는 [`scenario-contract-v1.md`](./scenario-contract-v1.md)(시나리오별 메시지)와 [`topic-convention.md`](./topic-convention.md)(공통 토픽·봉투 규칙)를 **처음 보는 사람도 이해하도록** 우편/택배 비유로 풀어 쓴 설명판입니다.
...
셋째 칸에 아무 말이나 쓸 수는 없습니다. 이동 결과는 7개, 따라가기 결과는 5개의 정해진 낱말만 받습니다. 두 목록은 겹치기는 해도 서로 포함 관계가 아니므로 **한쪽 낱말을 다른 쪽에 쓰면 편지가 통째로 버려집니다.** 정확한 목록은 [`topic-convention.md`](./topic-convention.md) §9에 있습니다.
...
- 시나리오별 메시지 의미가 바뀌면 → [`scenario-contract-v1.md`](./scenario-contract-v1.md)를 먼저 고칩니다.
- 공통 토픽·봉투 규칙이 바뀌면 → [`topic-convention.md`](./topic-convention.md)를 함께 고칩니다.
