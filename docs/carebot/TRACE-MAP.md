...
> 직접 I/O 를 하지 않는다는 원칙(CLAUDE.md §23)을 지키면서, 생성 호출은 여전히 턴당
> 한 번입니다(§16).
>
> ```
...
**"줄" 열은 폐기했습니다.** 예전 판에는 상수마다 줄 번호가 붙어 있었는데, 2026-08-15 실측에서
28행 중 26행이 틀렸습니다. 커밋 하나마다 다시 틀리는 열이라 유지 비용이 값어치보다 큽니다.
상수명은 저장소에서 유일하므로 `policy.py` 안에서 이름으로 찾으십시오.
...
| `T3_CONSENT_DELAY_SEC` | 45분 | 속마음과 동의 질문 사이 여운이 늘어남 | 감시처럼 느껴질 위험 (§CLAUDE.md §9) |
...
| `OUTBOX_MAX_ATTEMPTS` | 티어별 | — | **T1 은 무제한이어야 합니다** |
