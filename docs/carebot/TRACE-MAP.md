...
| 4 | 물음표로 끝나고 의료 질의 | `info` | `llm/router.py` 의 **키워드 결정 규칙**에 위임 (여기서만). 임베딩 모델은 실측 후 제거됨(d7ce99a) — 아래 4-4 의 로딩 지연 문제가 근본적으로 사라진 이유 |
...
| 6 ★ | `"대전 날씨 어때"` | `info` | `none` | `context_read` 조회 → `handle_info` | 실제 관측값. §2.5 참고 |
| 7 ★ | `"오늘 몇 도야"` | `info` | `none` | `handle_info` | 🔴 **도시 없음 → 조회 안 함. LLM 이 지어냄** |
