...
코드에 반복해서 나타나는 판단들입니다.

### 3.1 말하지 않기로 정하는 것도 기능이다

타이머가 울릴 때마다 말하는 로봇은 잔소리꾼이 됩니다. 새벽 3시에 떠들고, TV 를 끊고, 5분 전 알림을 또 합니다.

그래서 **아무도 직접 말하지 않습니다.** 스케줄러·사다리·현관·재질의가 전부 '제안'만 하고, 게이트가 심판합니다. 게이트가 침묵을 고르면 그래프는 `END` 로 직행하며 **`emit` 에 도달하지 않습니다** — 빈 응답을 나중에 걸러내는 게 아니라, 애초에 만들지 않습니다.
...
**T1 은 시도 횟수로 포기하지 않습니다.** 어제의 일일 요약은 20번 만에 포기하지만, 생명 안전 알림은 큐에 남아 있는 것 자체가 알려야 할 상태입니다.
...
저장소를 나눠서 두 가지를 동시에 달성합니다 — 이것이 §3.3 의 "저장 위치를 나눈다"가 실제로 쓰이는 대표적인 예입니다.
...
`speaking` / `spoken_prefix` 는 주인이 둘입니다 — 재생 스레드와 checkpoint 된 state. CLAUDE.md 가 **"시스템에서 동기화 버그가 가장 나기 쉬운 지점"** 이라고 못박은 곳입니다.
...
T4 는 알림 등급이 아니라 **기억의 공개범위**(`memory.visibility = PRIVATE`)로 표현됩니다.
...
현재 2번과 10번이 약합니다(의미 검색이 꺼져 있어서 — 218 에서 만들어졌지만 임베딩
API 과금 때문에 기본값이 off 입니다. "없어서"가 아니라 "꺼져 있어서"입니다). →
[PROGRESS.md](PROGRESS.md) §2.4
...
**성능 저하 순서**를 미리 정해뒀습니다: 기억 top-k 축소 → 문서 RAG 중단 → 잡담 중단 → 프로브 단순화. **안전 경로는 마지막까지 살립니다.**
