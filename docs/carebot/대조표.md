...
flowchart TD
    ...
    START([턴 시작]):::term --> R{trigger_type}:::gate
    R -->|user_utterance| NI[note_interaction<br/>🔵 침묵 0 · 재실 HOME]:::robot
    R -->|proactive| PG[proactive_gate<br/>제안 중 최대 1건만 통과]:::gate
    R -->|door_event| DE[door_event<br/>🔵 재실 UNKNOWN]:::sensor
    R -->|backend_command| BC[backend_command<br/>⚪ 명령을 state 로]:::be

    DE --> ENDA([END]):::term
    PG -->|silent| ENDB([END]):::term
    NI -->|맞장구| ENDC([END]):::term

    NI -->|그 외| ST[safety_triage<br/>🔵 confirm 이면 마감시각]:::safety
    ST -->|T1| ESC[escalation<br/>🔵 outbox + 억제기록]:::safety
    ST -->|confirm| SC[safety_confirm]:::safety
    ST -->|none| CI

    PG -->|speak| CI[classify_intent<br/>⚪ 순수 규칙]:::gate
    BC --> CI
    CI --> CR[context_read<br/>🟣 문맥 조회 · 🔵 캐시]:::be
    CR --> H[handle_* 7개]:::robot

    ESC --> RS[response_shaper<br/>⚪ 마지막 관문]:::robot
    SC --> RS
    H --> RS
    RS --> EM[emit<br/>⚪ 논블로킹 재생]:::hw
    EM --> MW[memory_write<br/>🟣 턴 2행 · 🔵 3곳]:::be
    MW --> ENDD([END]):::term
```
...
| `availability.semanticSearch` | **이 환경의 `EMBEDDING_ENABLED` 와 같은 값.** 코드 기본값·로컬 compose 는 `false`, 시연 서버(`i15e102`)는 2026-08-10 부터 `true` ([진행 상황.md](<진행 상황.md>) §2.4) | 스위치와 값이 어긋나면 거짓 보고 |
...
| 문서 | 언제 |
|---|---|
| [검증 절차.md](<검증 절차.md>) | **명령을 돌려 확인하고 싶을 때.** 이 문서가 "무엇이 도는가"라면 저쪽은 "어떻게 확인하는가"입니다 |
| [진행 상황.md](<진행 상황.md>) | 지금 무엇이 안 되고 있는가 |
| [개념과 설계 판단.md](<개념과 설계 판단.md>) | 왜 이렇게 만들었는가 |
| [코드 읽는 순서.md](<코드 읽는 순서.md>) | 처음 오셨다면 어디부터 |
| [../hardware/오디오 에코 바지인 검증.md](<../hardware/오디오 에코 바지인 검증.md>) | 에코·말끊기 임계치를 실기로 정할 때 |
