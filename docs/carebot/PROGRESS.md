...
> **마지막 갱신**: 자연스러운 대화 감사·계획 문서 작성 직후 (2026-08-06)
>
...
| **233** | **실기 통합 점검** | 체크리스트 + 준비단계 수정 6건, **문서 3종 + 상태 도구** | 통과 | **부분 실시** (준비단계만, 본검사 0~11 미완) | ai-develop |
...
| 대상 | lint | 테스트 |
|---|---|---|
| `ai-develop` + 341 (2026-08-05 측정) | `All checks passed` | `655 passed in 19.58s` |
...
| 무엇 | 어디 | 역할 |
|---|---|---|
| 본문 (읽기 전용) | [`FIELD-TEST-233.md`](FIELD-TEST-233.md) | 0~11절. 스텝마다 **함수 호출 순서·DB 접근 지점·API 방향**을 표로 |
| 결과지 (적는 곳) | [`FIELD-TEST-233-RESULT.md`](FIELD-TEST-233-RESULT.md) | 판정이 **일치 / 불일치 / 확인 못함** 3택 |
| 대조표 | [`TRACE-MAP.md`](TRACE-MAP.md) | 발화 42개 → 함수 → DB → API → 조절값 |
| 상태 도구 | `robot/ai_chat/tests/manual/probe.py` | `--save` / `--diff` 로 **발화 하나가 바꾼 것만** 표시 |

**판정을 3택으로 만든 것이 의도입니다.** 못 본 것을 "일치"로 적으면 다음 사람은 없는 문서가
아니라 **틀린 문서**를 읽습니다. 결과지 마지막에 "확인 못한 항목" 칸을 따로 두었습니다.
...
결함이 나왔습니다. 전부 로직으로 확인했고 아직 고치지 않았습니다 — Phase 1
(세션 계층)과 이후 Phase 에 배정되어 있습니다.

| # | 결함 | 위치 | 영향 |
|---|---|---|---|
| 1 | `speaking` 이 재생 정상 종료 시 안 내려감 | `output.py:299` 쓰기, 복원은 바지인 경로뿐 | 2번째 턴부터 모든 발화가 바지인으로 오분류 (로그·의도 왜곡) |
| 2 | `interrupted_remainder` 소비처 없음 | 쓰기 `ingress.py:222`, 읽기 0곳 | 끊긴 발화 나머지가 영원히 재발화 안 됨 |
| 3 | `proposal_meta` writer 없음 | 읽기 `handlers.py:306` 뿐 | 능동 복약 알림 직후 "약 먹었어" 완료 보고가 슬롯을 못 찾을 수 있음 |
| 4 | `audio_ctx` writer 없음 | 게이트가 읽기만 (`gate.py:290`) | 게이트 4(끼어들기 방지)가 항상 통과 — 사실상 비활성 |
| 5 | T2 적재 5곳에 동의 확인 없음 | `ticks.py:474,526,573,592` 외 | `outbox.py:64-67` 이 요구하는 규칙이 코드에 없음. 서버 최종 방어선에만 의존 |
| 6 | 로봇 최종 발화 전문이 로그 파일에 남음 | `output.py:262` (INFO → `ai_chat.log` 20MB×5) | 원문 미기재 보증이 보호자 알림 payload 에만 걸려 있음 |
| 7 | 어르신 발화 원문이 stdout 으로 출력 | `bootstrap.py:546` `print` | 로그 파일엔 안 남지만 콘솔 캡처(journald·tmux)에 남음 |
