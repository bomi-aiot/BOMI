...
| **227** | **BE 온보딩·재질의 계약 API** | 완료 | 통과 | 해당 없음 | 푸시됨 |
...
**자동 검증 현황**: 로봇 375건 통과 / 백엔드 138건 통과, 둘 다 lint 통과.
...
### 2.6 로봇→백엔드 '쓰기' 경로 — 절반이 채워졌습니다

203 이 만든 것은 **읽기 전용 문맥 조립**뿐이었습니다. 227 이 온보딩·재질의를 채웠고, 나머지 둘은 아직 없습니다.

| 경로 | 상태 |
|---|---|
| 온보딩 세션 진행·답변 제출 | ✅ 227 |
| 활성 `fact_candidate` 조회·응답 | ✅ 227 |
| 대화 이벤트 적재(`conversation_message`) | ❌ 211 의 발화량 지표가 통째로 NULL |
