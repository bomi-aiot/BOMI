...
읽기 전에 [개념과 설계 판단.md](<개념과 설계 판단.md>) 의 §1(용어)만 훑으면 훨씬 수월합니다.
...
| 5 | `docs/carebot/진행 상황.md` | 지금 무엇이 되고 무엇이 안 되는지 |
| 6 | `docs/natural-conversation/현재 구조 감사.md` | **2026-08 이후 작업의 출발점.** 무엇이 이미 있고(세션·게이트) 무엇이 없는지(문맥 슬롯·참조 해소·기억 삭제), 파일:라인 근거로. 이어서 같은 폴더의 implementation-plan(Phase 1~7)·target-architecture 를 읽습니다 |
...
flowchart TD
    ...
    A["graph/turn.py<br/>run_user_turn"]:::term --> B["ingress.note_interaction<br/>사다리 리셋 · occupancy=HOME · barge-in"]:::robot
    B -->|맞장구| Z1([END]):::term
    B --> C["triage.safety_triage"]:::safety
    C -->|T1| ESC[escalation]:::safety
    C -->|confirm| SC[safety_confirm]:::safety
    C -->|none| D["context.classify_intent<br/>로컬 규칙, LLM 안 씀"]:::gate
    D --> E["context.context_read<br/>백엔드 문맥 · 실패 시 캐시"]:::be
    E --> F["handlers._generate<br/>이 턴의 유일한 LLM 호출"]:::robot
    F --> G["prompts/builder.build_prompt"]:::robot
    G --> H["output.response_shaper<br/>문장 분할 · 개수 제한"]:::robot
    ESC --> H
    SC --> H
    H --> I["output.emit<br/>재생 시작 (논블로킹)"]:::hw
    I --> J["build.memory_write<br/>턴 기록 (블로킹 HTTP)"]:::be
    J --> Z2([END]):::term
    I -.재생.-> K["audio/playback.py"]:::hw
```
...
| 순서 | 파일 | 여기서 볼 것 |
|---|---|---|
| 1 | `docs/database/mvp-erd.md` §9 | 문맥 조립 레시피. **이것이 계약이다.** 같은 폴더의 `column-definition/`·`Flyway 가이드.md` 는 필요할 때만 |
