...
| [2. 외부 API](#2-외부-api-단독-점검) | STT·TTS·LLM·마이크 빔 | 30분 | ✓ |
| [3. 에코](#3-에코--여기를-건너뛰면-이후가-전부-오염됩니다) | **먼저 잡습니다** | 40분 | ✓ |
| [4. 한 바퀴](#4-한-바퀴-돌리기) | 왕복·지연·**첫 질문 지연** | 40분 | ✓ |
| [5. 발화 매트릭스](#5-발화-매트릭스--이-점검의-본체) | **42개 발화 + 날씨·병원 심화** | 100분 | ✓ |
...
## 2-3. 🔴 마이크 빔이 실제로 도는가 (팀원 피드백)

**빔(beam)이란**: reSpeaker 같은 마이크 어레이는 마이크가 여러 개라, 소리가 오는 **방향**을
계산해 그쪽만 집중해서 듣습니다. 그 방향 조절을 `BeamController` 가 합니다. 이게 안 돌면
사방의 소리를 똑같이 받아들여, 먼 거리·소음 환경에서 인식률이 눈에 띄게 떨어집니다.

> **★ 지금 그래프 경로에는 배선(코드를 실행 경로에 연결하는 것)이 안 돼 있습니다.**
> `BeamController` 를 붙이는 줄은 `main.py:192` 의 **레거시 경로에만** 있고,
> 그래프 경로(`_run_graph_runtime`)에는 없습니다. 즉 지금 기본 실행에서는 빔 제어가
> 동작하지 않습니다.
>
> 232가 "만들어 놓고 아무도 안 부르는" 결함을 대거 찾아냈는데, **같은 유형이 여기 남아
> 있었습니다.** 팀원이 "무슨 말인지 이해를 못했음"이라고 적은 것은 자연스럽습니다 —
...
**기대**: `main.py` 에서만 나옵니다(= 미배선 확인). `bootstrap.py` 에도 나오면 누군가
고친 것이니 아래 실측으로 넘어갑니다.

**배선된 뒤에 잴 것** (지금은 건너뜁니다)

| 상황 | 확인 |
| --- | --- |
| 로봇 정면 1m 에서 말하기 | 인식률 기준선 |
| 로봇 측면 90° 1m 에서 말하기 | 빔이 따라오는가 |
| 3m 거리에서 말하기 | 원거리 성능 (소프트웨어로 못 고치는 한계일 수 있음) |
...
## 4-4. 🔴 첫 날씨·병원 질문만 유독 느린가 (팀원 피드백)

> **⚠️ 2026-08-06 갱신 — 이 스텝의 전제가 바뀌었습니다.** 아래가 문제 삼는 임베딩
> 모델(`ko-sroberta-multitask`)은 실측(기동 6.28s·메모리 약 732MB·정확도 이득 없음)
> 후 **운영 경로에서 제거**되었습니다(커밋 d7ce99a — 의도 판정은 키워드 규칙).
> 따라서 이 스텝의 기대 결과는 "첫 질문도 다른 턴과 지연 차이가 없어야 한다"로
> 바뀝니다. 여전히 첫 질문만 수 초 느리다면 그것은 새 결함입니다.
> 아래 원문은 제거 전 기록으로 보존합니다.

> **왜 이 스텝이 따로 필요한가.** 4-2 는 "안녕하세요" 같은 잡담으로 지연을 쟀습니다.
> 그런데 **날씨·병원 질문의 첫 한 번은 다른 경로**를 탑니다.
>
> 의도 판정에 쓰는 작은 AI 모델(`ko-sroberta-multitask`)이 있는데,
> [router.py:6](../../robot/ai_chat/src/bomi_ai_chat/llm/router.py:6) 이 이 모델을
> **모듈을 불러오는 순간** 메모리에 올립니다. 그런데 그 모듈을 불러오는 코드가
> `context.py` 의 함수 **안에** 있어서, **첫 의료·날씨 질문이 들어온 바로 그 순간**에
> 로딩이 시작됩니다 — 어르신이 대답을 기다리는 중에.
>
> **레거시 경로에는 이걸 미리 하는 줄이 있습니다** (`main.py:210` 의
> `_semantic_weather("워밍업")`). **그래프 경로에는 없습니다** — `bootstrap.py` 에
> `router` 라는 글자가 한 번도 안 나옵니다.
>
> 그래서 팀원이 본 순서가 이렇게 됩니다: 첫 턴 대답이 나가고(TTS) → 다음 턴에 병원을
> 물으면 → **거기서 모델이 로딩**되며 몇 초가 더 걸립니다. "말한 다음에 불러오니까
> 답변 만드는 시간에 모델 불러오는 시간이 합쳐진다"는 관찰이 정확합니다.

**말할 것** (로봇을 **새로 띄운 직후**, 순서대로)

| # | 말할 것 | 재는 것 |
| --- | --- | --- |
| 1 | `"안녕하세요"` | 기준선 (모델 로딩 없음) |
| 2 | `"대전 날씨 어때"` | **여기서 모델이 로딩됩니다** |
| 3 | `"부산 날씨 어때"` | 두 번째부터는 빨라야 정상 |

**기대**

| 확인 | 미배선 상태(지금) | 워밍업이 붙은 뒤 |
| --- | --- | --- |
| 2번이 1번보다 | **몇 초 더 느림** | 비슷 |
| 3번이 2번보다 | **뚜렷하게 빠름** | 비슷 |

2번과 3번의 차이가 곧 **모델 로딩에 걸린 시간**입니다. 그 값을 적어 두면 워밍업을
붙일 가치가 있는지 숫자로 판단할 수 있습니다.
...
| 증상 | 어디 | 무엇을 바꾸면 |
| --- | --- | --- |
| 2번이 유독 느림 | `bootstrap.py` 에 워밍업 없음 | 기동 시 `router.is_medical_query("워밍업")` 을 한 번 불러 미리 로드 |
| 세 번 다 느림 | 네트워크 | 4-2·4-3 으로 |
| 1번부터 느림 | 웨이크워드 모델 | `WAKEWORD_ENABLED` 확인 |
...
## 5-3. `"대전 날씨 어때"` vs `"오늘 몇 도야"` — 🔴 **도시명 하나로 갈립니다**

> **날씨는 311 에서 배선됐습니다. 다만 조건부입니다.**
>
> 조회는 `handle_info` 가 아니라 **`context_read` 가** 합니다
> ([context.py](../../robot/ai_chat/src/bomi_ai_chat/graph/context.py) `_lookup_weather_documents`).
> 기상청에서 받아온 값을 `ctx["documents"]` 에 "참고 자료"로 넣어 두면, `handle_info` 는
> 그걸 읽어 한 문장으로 말합니다. 핸들러를 얇게 유지하려는 설계입니다 — 핸들러는
> 무엇을 말할지만 정하고, 조회 같은 I/O 는 하지 않습니다.
>
> **★ 그런데 도시를 특정하지 못하면 조회 자체를 시도하지 않습니다.** `extract_city()` 가
> 빈 값을 돌려주면 그대로 `[]` 를 반환합니다. 참고 자료가 없으니 모델은 아는 대로
> 말하고, **그 숫자는 지어낸 것입니다.**
>
> **왜 이 스텝이 중요한가:** 로봇이 자연스러운 한국어로 기온을 말하면 점검자는 ✅ 를
> 찍고 넘어갑니다. **자연스러움이 정확함처럼 보이는 것**, 이것이 이 제품에서 가장 잡기
> 어려운 종류의 실패입니다. 두 문장을 나란히 해야 갈리는 지점이 보입니다.

**말할 것** ▶ `"대전 날씨 어때"` (도시 있음) → 이어서 ▶ `"오늘 몇 도야"` (도시 없음)

| 발화 | 기상청 호출 | 로봇의 답 |
| --- | --- | --- |
| `"대전 날씨 어때"` | **함** | 실제 관측값 기반 |
| `"오늘 몇 도야"` | **안 함** | 🔴 **LLM 추측** |
...
**기대**: 첫 발화에서만 조회 로그가 남습니다.

**판정**

- ☐ 도시가 있을 때만 조회함 (설계대로)
- ☐ 둘 다 조회 안 함 → 311 배선이 안 도는 것. `_WEATHER_MARKERS` 와 `extract_city` 확인
...
### 5-3b. 🔴 조회에 성공해도 말이 어색한가 (팀원 피드백)

> **팀원 관찰**: *"자연스럽지 않은 문장이고 뭔가 `온도 : 몇도` 이런식으로 말함."*
>
> **원인은 모델이 아니라 우리가 넘긴 자료의 모양입니다.**
> [context.py:384](../../robot/ai_chat/src/bomi_ai_chat/graph/context.py:384) 가 기상청
> 응답을 이렇게 문자열로 만듭니다.
>
> ```python
> summary = ", ".join(f"{label} {value}" for label, value in forecast.items())
> # → "기온 3, 하늘상태 1, 강수형태 0, 강수확률 20"
> ```
>
> 문제가 셋입니다.
>
> | 무엇 | 왜 문제인가 |
> | --- | --- |
> | `하늘상태 1` | 기상청 **숫자 코드**입니다(1=맑음, 3=구름많음, 4=흐림). 모델은 이게 무슨 뜻인지 모릅니다 |
> | `기온 3` | **단위가 없습니다.** 3도인지 3퍼센트인지 자료만 봐서는 알 수 없습니다 |
> | 라벨-값 나열 | **그대로 읽으라고 유도하는 모양**입니다. 그래서 "기온은 3입니다"가 나옵니다 |
>
> CLAUDE.md §14 는 정반대를 요구합니다 — **"3도예요"가 아니라 "추우니 내복
> 챙기세요"**. 정보가 아니라 행동으로 말해야 합니다. 지금은 자료 모양이 그 규칙과
> 정면으로 부딪힙니다.

**말할 것** ▶ `"대전 날씨 어때"` → `"부산 오늘 추워?"` → `"광주 비 와?"`

**기대와 실패의 모습**

| | 좋은 답 | 지금 나올 가능성이 큰 답 |
| --- | --- | --- |
| 형태 | `"대전은 쌀쌀해요. 나가실 거면 겉옷 챙기세요."` | `"기온 3, 하늘상태 1입니다."` |
| 숫자 | 없거나 자연스럽게 하나만 | 코드값이 그대로 |
| 행동 | 있음 | 없음 |

**확인 항목**

- ☐ 숫자 코드(`하늘상태 1` 같은)를 소리 내어 말하지 않습니다
- ☐ 단위 없는 숫자를 말하지 않습니다
- ☐ 온도만 읽지 않고 **무엇을 하면 되는지**가 한마디 붙습니다
- ☐ 한 문장 안에 지표를 3개 이상 나열하지 않습니다
...
**결과지** ▶ **5-3b** (로봇이 한 말과 참고 자료 원문을 **둘 다** 적습니다)

**안 맞으면 여기**

| 증상 | 어디 | 무엇을 바꾸면 |
| --- | --- | --- |
| 코드값을 그대로 말함 | `context.py` `_lookup_weather_documents` | 코드→한국어 변환표를 넣고(`1→맑음`), 단위를 붙입니다(`3℃`) |
| 지표를 줄줄이 나열 | 같은 곳 | 넘기는 지표를 기온·강수 정도로 줄입니다. 안 넘기면 말할 수 없습니다 |
| 행동 제안이 없음 | `prompts/templates/system.md` | §14 의 "정보가 아니라 행동" 규칙이 프롬프트에 있는지 |

---

## 5-3c. `"남경의원 알려줘"` — 312 회귀 확인 (팀원 피드백)

> **팀원이 겪은 것**: *"남경의원 알려줘 → 병원 관련 질문으로 인식을 못함 + db 못
> 긁어옴 + 했던 답변을 기억 못함."*
>
> **원인 두 가지가 밝혀졌고 둘 다 312 에서 고쳤습니다.** 이 스텝은 그 수정이 실기에서
> 실제로 되는지 확인하는 회귀 점검입니다.
>
> | 무엇이 문제였나 | 왜 그랬나 |
> | --- | --- |
> | 병원 질문으로 인식 못 함 | `"남경의원 알려줘"` 는 물음표로 끝나지 않습니다. 예전 코드는 **물음표로 끝나는 문장만** 의료 판정기에 넘겼습니다. 그래서 잡담으로 샜습니다 |
> | DB 를 못 긁어옴 | DB 에는 `"남경 의원"`(공백 있음), 질의는 `"남경의원"`(공백 없음). 부분 문자열 매칭이 실패했습니다 |
> | 했던 답변을 기억 못 함 | **기억 문제가 아니었습니다.** 로봇이 `"남경 의원"`이라고 말해 놓고, 어르신이 되물으면 공백 없이 넘어와 **방금 자기가 알려준 곳을 못 찾겠다고** 답한 것입니다 |
>
> 마지막 항목이 이 피드백에서 가장 값진 발견입니다. "기억을 못 한다"는 증상이 실제로는
> **문자열 공백 하나**였습니다. 증상만 보고 기억 기능을 파고들었다면 한참을 헤맸을
> 것입니다.
...
| # | 말할 것 | 확인하는 것 |
| --- | --- | --- |
| 1 | `"근처 병원 알려줘"` | 목록이 나오는가 |
| 2 | (로봇이 말한 병원 이름 하나를 그대로) `"○○의원 알려줘"` | **방금 알려준 곳을 다시 찾는가** |
| 3 | `"거기 몇 시까지 해?"` | 이어지는가 |
| 4 | `"부산 강서구 정형외과 찾아줘"` | 물음표 없는 문장 |

**기대**

| 확인 | 정상 | 실패 |
| --- | --- | --- |
| 갈래 | `info` (의료도 `info` 로 갑니다 — 정상입니다) | `companion` 이면 판정 실패 |
| 조회 로그 | `lookup documents=N medical=True` | `medical=False` 또는 `documents=0` |
| 2번 답변 | 그 병원 정보를 말함 | 🔴 **"못 찾겠다"면 312 회귀** |
| 목록 개수 | 2~3곳까지 말함 | `"찾아드릴게요"` 만 하고 안 말하면 🔴 프롬프트 충돌 재발 |
...
| 증상 | 어디 | 무엇을 바꾸면 |
| --- | --- | --- |
| 갈래가 `companion` | `context.py` `_MEDICAL_HINT_MARKERS` | 병원 이름 형태(`의원`·`병원`·`한의원`)가 목록에 있는지 |
| `documents=0` | `db/medical_repository.py` | DB 연결(`DB_CONNECTION_MODE`)과 공백 정규화가 살아 있는지 |
| 목록이 있는데 말을 안 함 | `prompts/templates/medical_stance.md` | "2~3개까지 나열 가능" 지시가 붙었는지 (312가 넣은 파일) |
| 방금 알려준 곳을 못 찾음 | `medical_repository.py` 공백 정규화 | 🔴 312 회귀입니다 |
