...
검증 상태: **698 passed + ruff clean** (2026-08-06 실측, `-m "not integration and not manual"`)
...
| Phase 3: 프로필 미사용 필드(conversationPreferences·chronicPainArea) 프롬프트 반영 + 주소 폴백(시나리오 C 준비) | 완료 (**커밋 필요 — 아래 재개 1번**) | — |
| Phase 5-1: T4 봉인 전 인텐트 확장 + "기억하지 마"(봉인+대기 행 삭제) + 테스트 9건 | 완료 (**커밋 필요**) | — |
...
2. **CLAUDE.md 재구성** (작업 목록 #5, 유일한 미완 큰 덩어리):
   ...
   - **§30 신설**: 문맥 선택 우선순위(코드가 이미 `CLAUDE.md §30` 을 인용 중 —
     `policy.py` 문맥 다이얼 절, `context_slots.py`, `context.py`) ← **이게 없으면 코드 주석이 깨진 참조다. 최우선.**
