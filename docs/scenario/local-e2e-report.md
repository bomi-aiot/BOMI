...
> **과거 로컬 simulator 검증 기록입니다. 현행 실물 연동 절차나 최종 메시지 계약의 정답지로 사용하지 마십시오.**
> 실물 연동은 [`integration-guide.md`](./integration-guide.md)와 [`scenario-contract-v1.md`](../mqtt/scenario-contract-v1.md)를 따르며,
> 실물 Robot Bridge와 `robot-sim`을 같은 `robotId`로 동시에 실행하지 않습니다.
