...
```mermaid
flowchart TD
    A["로봇이 멈춰 있다"]:::neutral --> B{"활성 Scenario 가 있나?"}:::decision
    B -- "없음" --> C["§2 mode 복구만 호출<br/>(SAFE_STOP/SCENARIO_ACTIVE → IDLE)"]:::api
    B -- "1개" --> D["§1 취소 호출<br/>→ CANCELLED + SAFE_STOP"]:::danger
    B -- "2개 이상" --> E["409. 자동 처리 불가<br/>원인부터 조사"]:::danger
    D --> F{"Jetson/Nav2 에서<br/>실제 정지 확인?"}:::decision
    F -- "예" --> C
    F -- "아니오" --> G["여기서 멈춘다.<br/>로봇 담당자 호출"]:::danger
    C --> H["IDLE. 시나리오 재개 가능"]:::ok
    E --> G

    classDef neutral fill:#e7e7e7,stroke:#595959
    classDef decision fill:#fff6b6,stroke:#af7e02
