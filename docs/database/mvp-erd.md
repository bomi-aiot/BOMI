...
> 상태: 설계 기준안 · PostgreSQL 17 / pgvector · 2026-07-22
> 범위: 귀가 환영 및 맞춤 대화 MVP를 우선하는 8개 테이블
...
| 로봇 현재 배정, 마지막 통신, 배터리, waypoint 참조 | 최신 상태만 저장 | 배정의 원본은 Backend, 실제 pose·경로·안전 판정의 원본은 ROS 2/Nav2이다. |
| 출입·귀가·낙상 시나리오 | 의미 있는 단계와 최종 결과 저장 | 장치가 `external_event_id`와 `occurred_at`을 만들고 Backend가 수신·멱등 처리 결과를 소유한다. |
| Vision 결과 | 사람 수, 상호작용 가능 여부 등 최종 업무 판정만 저장 | 프레임·track의 원본은 Vision이며, track ID를 사람 ID로 승격하지 않는다. |
...
| 건강·복약·일정·알림 | `care_record`의 유형별 업무 행으로 저장 | 계획과 실행 결과를 다른 행으로 만들고 `parent_record_id`로 연결한다. |
| 민감정보 변경 행위 | 본문 없이 변경 필드명과 행위만 저장 | `audit_log`는 변경 전후 민감값을 복제하지 않는다. |
| LiDAR, 카메라 프레임, 영상 스트림, 원본 음성, 초당 pose, 모터값 | 저장하지 않음 | 로봇/Vision/음성 시스템에서 실시간 처리 후 폐기하거나 각 시스템의 운영 로그 정책을 따른다. |
...
2. DB 세션과 애플리케이션은 UTC를 기준으로 쓰고, 사용자 시간대 표시는 UI에서 변환한다. 모든 사건 시각은 `timestamptz`다.
...
LiDAR 원시 데이터, 카메라 프레임·실시간 영상, 원본 음성 장기 보관, 초당 로봇 좌표, 모터 제어값, 장애물 회피 계산 전 과정, Vision 프레임별 결과, STT 중간 청크, TTS 내부 단계, 전체 HTTP 요청·응답, 전체 프롬프트/응답의 중복 로그, 얼굴 특징·얼굴 임베딩·생체정보는 저장하지 않는다. 디버깅 편의를 이유로 `timeline`, `messages`, `details`, `audit_log`에 우회 저장하는 것도 금지한다.

### 8개로 통합한 이유와 의도적으로 미룬 정규화

- `app_user`: 어르신과 보호자의 공통 개인정보를 한곳에 둔다. 어르신 로그인 컬럼의 NULL을 허용하는 대신 계정·프로필 분리를 미룬다.
- `care_relationship`: N:M 연결과 우선순위·소수 권한을 한 행으로 관리한다. 역할/권한 기준정보 테이블을 만들지 않는다.
- `robot`: 장치, 현재 배정, 최신 상태, 지도·waypoint 참조를 합친다. 초당 상태와 배정 이력은 저장하지 않는다.
...
- `care_record`: 건강, 복약, 일정, 인지 참고 지표와 보호자 알림을 `record_type`으로 구분한다. NULL이 많은 넓은 행을 의도적으로 허용한다.
...
| 항목 | 결과 |
| --- | --- |
| 실제 현재 테이블 | `app_user`, `care_relationship`, `robot`, `scenario`, `conversation`, `memory`, `care_record`, `audit_log` |
| 테이블 수 | **8개** |
| 8개 이하 | 예 |
| 8개 초과 | **아니오** |

현재 제외·통합한 후보는 `device`, `device_event`, `robot_assignment`, `robot_state`, `navigation_mission`, `vision_result`, `scenario_step`, `conversation_message`, `conversation_summary`, `memory_embedding`, `medication`, `schedule`, `notification`, `ai_execution`, 도메인별 감사 테이블이다. 현재 상태 컬럼, 명령·요청 ID, 제한된 JSONB, 자기참조 FK와 유형별 `care_record` 행으로 처리한다.

완전한 범용 Transactional Outbox는 이 8개에 포함하지 않는다. MVP에서는 `scenario`의 명령 ID와 `*_status=REQUESTED`를 커밋한 뒤 해당 행을 재조회하여 동일 명령 ID로 재발행한다. 여러 채널·다수 메시지·장기 재시도가 필요해 이 방식의 누락 또는 잠금 경합이 측정되면 Outbox를 우선 분리해야 한다.
...
| `conversation_preferences` | `jsonb` | N | `{"schemaVersion":1}` | JSON object CHECK | 제한된 대화 선호 | Y | 사용자 |
...
| `current_mode` | `varchar(40)` | N | `IDLE` | 서비스 허용값 | 최신 업무 모드 | 간접 | 최신값만 |
...
삭제 정책: 로봇 폐기는 `status=RETIRED`, 배정 해제는 `is_active=false`로 처리한다. 사용자 삭제 시 로봇 행은 유지하되 `senior_id`와 주거공간을 유추할 waypoint를 제거한다. 초당 상태·pose 이력은 생성하지 않는다.
...
`memory_type` 허용값은 `PERSONAL_RELATIONSHIP`, `PREFERENCE`, `HOBBY`, `LIFE_EVENT`, `FAMILY_MEMORY`, `EMOTIONAL_EVENT`, `CONVERSATION_SUMMARY`, `OTHER`다. 검증 상태는 `UNVERIFIED`, `AUTO_ACCEPTED`, `USER_CONFIRMED`, `GUARDIAN_CONFIRMED`, `REJECTED`다.
...
목적: 건강·복약·일정·인지 참고 지표·보호자 알림을 유형별 독립 행으로 저장하고 부모 행으로 계획과 결과를 연결한다.
...
| `external_event_id` | `varchar(64)` | Y | — | 부분 UNIQUE, 형식 allowlist | 오프라인 복약/안전 결과 멱등 키 원문 | 간접 | 행과 동일 |
| `record_type` | `varchar(40)` | N | — | 허용값 CHECK | 건강·약물·일정·실행·알림 종류 | Y | 유형별 정책 |
...
| `verification_status` | `varchar(30)` | N | `UNVERIFIED` | 서비스 허용값 | 확정 여부 | Y | 유형별 정책/감사 |
...
`record_type`은 `HEALTH_CONDITION`, `ALLERGY`, `PHYSICAL_LIMITATION`, `MEDICATION`, `MEDICATION_SCHEDULE`, `MEDICATION_REMINDER`, `MEDICATION_TAKEN`, `APPOINTMENT`, `PERSONAL_SCHEDULE`, `HEALTH_OBSERVATION`, `COGNITIVE_ASSESSMENT`, `GUARDIAN_NOTIFICATION`으로 시작한다.

삭제·보존 정책: 해소된 `HEALTH_OBSERVATION`은 30일, 미해소 관찰은 90일 후 재확인한다. 종료된 일반 일정·실행 결과는 기본 90일이다. 확정 질환·알레르기·복약 기준정보는 유효한 동안 유지하고 변경·삭제를 감사한다. 인지 결과에는 의료 진단이 아니라 참고 지표임을 `details`에 구조화한다. 낙상 알림은 안전 정책을 따른다. 사용자 삭제 시 법적 보존 근거가 없는 건강 본문과 JSONB를 물리 제거한다.
...
| FK 범주 | 권장 동작 | 이유 |
| --- | --- | --- |
| 소유자 `senior_id` in relationship/scenario/conversation/memory/care_record | 기본 `RESTRICT` | 삭제 오케스트레이션이 본문·임베딩·법적 보존을 먼저 판정해야 한다. |
...
| `memory.source_conversation_id` | `ON DELETE SET NULL` | 원문 만료 때는 conversation 행을 유지한다. 세션 자체의 물리 삭제 때만 출처 FK가 사라진다. |
| `care_record.recipient_guardian_id`, `created_by_user_id`, `audit_log.actor_user_id` | `ON DELETE SET NULL` | 결과/감사 행은 남기되 삭제 사용자의 직접 식별자를 제거한다. |
| `parent_record_id`, `superseded_by_id` 자기참조 | 기본 `RESTRICT` | 계획-결과와 기억 대체 계보를 보존한 뒤 명시적으로 정리한다. |
...
필수 키는 `schemaVersion: integer`다. 나머지는 선택 키이며 범위는 애플리케이션에서 검증한다.
...
- 허용: record type별 소수의 구조화된 부가정보. 약물명·용량은 care_record의 민감 건강정보로 저장할 수 있지만 **임베딩하지 않는다**.
- 금지: 비밀번호·토큰, 보호자 전화/이메일 복제, 대화 원문, 얼굴·생체정보, 전체 모델 응답·HTTP body, 범용 임의 이벤트 payload.
...
| `uq_care_record_external_event` | UNIQUE `(external_event_id)` WHERE NOT NULL | 오프라인 복약·안전 결과 중복 방지 |
...
`care_record.senior_id`, `scenario.senior_id/robot_id`, `conversation.senior_id`, `memory.senior_id`, `audit_log.actor_user_id`는 위 복합 인덱스의 선두 컬럼으로 FK 조회를 지원한다. 선택도가 낮은 상태 컬럼 단독 인덱스는 만들지 않는다.
...
| 1 | `app_user`, `care_relationship`, 최소 `audit_log` | 어르신/보호자 등록, 로그인 보호자 이메일 UNIQUE, 활성 관계·PRIMARY 제약, 관계 기반 authorization | 연결되지 않은 보호자의 조회가 403이고 PRIMARY 중복이 DB에서 거부됨 |
| 2 | `robot` | 로봇 등록, 어르신 현재 배정, 현관·기본 waypoint, last_seen과 배터리 | 어르신별 활성 로봇 한 대 조회 및 이중 배정 거부 |
| 3 | `scenario` 이벤트 수신 | `external_event_id` UPSERT, occurred/received 시각, 진행/종료 전이, 낙관적 잠금 | 같은 센서 이벤트를 반복 보내도 scenario ID가 하나 |
| 4 | `scenario` 주행·Vision·복귀 | 명령/request ID 선저장, 현관 이동, 최종 Vision 판정, 안전 조건 후 기본 위치 복귀/SAFE_STOP | Mock E2E에서 실패·timeout·늦은 결과가 종료 상태를 되돌리지 않음 |
| 5 | `conversation` | 최근 12턴, 롤링 요약, LOCAL/CLOUD fallback, 시나리오당 한 세션 | 맞춤 인사→대화→요약이 연결되고 보호자 API가 원문을 반환하지 않음 |
| 6 | `memory` | 추출 후보, 검증/공개/생명주기, 정확 벡터 검색과 재점수 | 거부·만료·삭제·비공개 기억이 검색/보호자 조회에서 제외됨 |
| 7 | 기존 테이블 View/API | 대시보드의 로봇·최근 귀가·공유 요약·공유 기억 | 대시보드 전용 테이블 없이 관계별 결과 조회 |
| 8 | `care_record` 복약·일정 | 계획/알림/실행 별도 행, 오늘 범위 조회, 대화 건강 관찰의 미확정 저장 | 계획을 덮지 않고 복용/미응답 타임라인 조회 |
| 9 | 보존·삭제와 전체 `audit_log` | 7일 원문 purge, 30일 AI 단기 메타데이터 제거, 기억 tombstone, 민감 변경 감사 | 대화 삭제 후 미확정 기억·임베딩·요약 잔존 검사 통과 |
| 10 | `scenario` + `care_record` 낙상 | 오프라인 낙상 멱등 수신, 음성 확인, PRIMARY→SECONDARY 단계 알림 | 보호자별 전달·확인 결과와 SAFE_STOP을 끝까지 설명 가능 |
...
8개 모두 현재 요구로 정당화된다. 더 합치면 핵심 FK/UNIQUE, 서로 다른 삭제 정책 또는 반복 1:N을 잃는다. 더 나누면 현재 측정되지 않은 미래 요구를 위해 복잡도를 늘리게 된다. 각 Spring Entity는 넓지만 단일 책임을 설명할 수 있고, JSONB는 제한된 변형 데이터에만 사용되어 핵심 무결성을 숨기지 않는다.
...
4. scenario 생성 시 `scenario.senior_id`와 `robot.senior_id`가 당시 활성 배정으로 일치해야 한다. 이후 재배정돼도 과거 scenario의 두 FK는 변경하지 않는다.
5. `conversation.scenario_id`가 있으면 conversation과 scenario의 `senior_id`가 같아야 한다.
6. `memory.source_conversation_id`가 있으면 memory와 conversation의 `senior_id`가 같아야 하며 `source_message_id`가 원문 보존 중에는 해당 items에 존재해야 한다.
7. `care_record.parent_record_id`가 있으면 부모와 자식의 `senior_id`가 같고 허용된 record type 연결이어야 한다.
8. `care_record.scenario_id`가 있으면 두 행의 `senior_id`가 같아야 한다. `recipient_guardian_id`는 `GUARDIAN`이고 생성 시 대상 어르신과 활성 관계여야 한다.
9. 보호자 조회·수정은 요청마다 활성 관계를 다시 확인한다. 과거 알림의 recipient FK가 현재 접근 권한을 부여하지는 않는다.
10. 종료 시각은 시작/발생 시각보다 빠를 수 없고, 종료 상태와 필수 결과 컬럼 조합은 scenario/record type별 validator가 확인한다.

### 최초 요구사항 추적

| 최초 프롬프트 범위 | 현재 근거 | 판정 |
| --- | --- | :---: |
| 1. 현재 테이블 8개 이하 | B의 수 검증, D의 8개 ERD, E의 8개 데이터 사전 | 충족 |
...
| 8. 8개 테이블별 출발 구조·정책 | D 관계, E의 173개 요구 후보 컬럼 포함 데이터 사전 | 충족 |
...
| 15. 필수 조회 14개 | G 필수 조회 검증의 14개 접근 경로 | 충족 |
| 16. 실제 문제 기반 향후 정규화 | J의 발생 조건·현재 한계·분리·이전·FK/보존 | 충족 |
| 17. A~K 출력 순서·내용 | 문서의 A~K 11개 최상위 절 | 충족 |
| 18. 금지사항과 최종 단순화 | A 저장 금지, B/D 현재 8개, J 미래 후보 격리, K 2차 검토 | 충족 |
...
이 결정들이 남아 있어도 8개 테이블의 경계, PK/FK, 멱등 키, 접근 경로와 삭제 연쇄는 구현을 시작할 만큼 확정돼 있다.
