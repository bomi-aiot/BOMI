...
> 상태: **열려 있음.** `ECHO_GUARD_SEC`·`ECHO_VAD_THRESHOLD_MULTIPLIER` 는 2026-08-16 현재도 추정값이며 실측되지 않았습니다. 진행 상황의 정본은 [`docs/carebot/PROGRESS.md`](../carebot/PROGRESS.md) 입니다.
...
상세: [`docs/carebot/PROGRESS.md`](../carebot/PROGRESS.md) "T1 폭주" 절.
...
- Jetson(aarch64)에서 VAD 모델 로딩·추론 지연 측정 — 프레임마다 도는 경로입니다. 참고로 임베딩 모델은 첫 호출 로딩이 **36.1초 / 21.5초**로 실측돼 워밍업을 넣었습니다([`docs/carebot/PROGRESS.md`](../carebot/PROGRESS.md)). VAD 도 같은 함정을 확인하십시오
...
로직은 자동 테스트로 고정돼 있습니다. 다만 세 조건 중 둘은 현재 구조(반이중)에서 성립할 수 없고, 나머지 하나는 다른 방법으로 충족되고 있습니다. 실기 열을 채우려면 7번의 결정이 먼저입니다. 티켓 상태는 [`docs/carebot/PROGRESS.md`](../carebot/PROGRESS.md) 표를 정본으로 봅니다.
