...
현재 노트북 카메라 영상에서 YOLO11로 사람을 탐지하고 바운딩 박스, 신뢰도와 화면 기준 위치를 표시할 수 있다. 사람 추적, 보호대상자 선택, 상태 머신 및 ROS2 연동은 아직 구현하지 않았다.
...
사람이 없으면 `not_found`, 두 명 이상이면 `multiple_people` 상태를 표시한다. 사람 추적이나 보호대상자 선택은 포함하지 않으며, 여러 사람이 감지되면 모든 바운딩 박스는 표시하되 특정 사용자의 위치를 임의로 대표 결과로 선택하지 않는다.
