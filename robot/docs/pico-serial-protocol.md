...
아래는 아직 검증되지 않았다. 확정되면 이 문서를 고친다.

| 항목 | 상태 |
| --- | --- |
| 텔레메트리 50Hz | Pico가 20ms 안에 문자열을 만들 수 있는지 미검증. 안 되면 25Hz로 낮춘다 |
| 워치독 300ms | 계산으로 잡은 값. 50Hz 전송에서 오탐 정지가 없는지 미검증 |
| 유효 트레드 | `angular.z` 변환에 기하학적 트레드 0.257m를 쓴다. 스키드 스티어는 회전 시 바퀴가 옆으로 긁혀 유효 트레드가 더 크므로, 자이로로 실측해 보정해야 한다 |
