...
public class WakeWordCallOrchestrator {
    ...
    public void onWakeWordDetected(
        ...
        log.info("Wake-word call movement started: scenarioId={}, seniorId={}, robotId={}, "
                + "eventId={}", scenario.getId(), seniorId, robotDeviceId, eventId);
    }

    /** Applies one correlated v1 NAVIGATION_RESULT. Legacy results are never accepted here. */
    @Transactional
    public void onNavigationResult(
        ...
        if (scenario == null) {
            log.warn("Wake-word navigation result references unknown scenario: scenarioId={}",
                scenarioId);
            return;
        }
        ...
        if (!Objects.equals(robot.getDeviceId(), sourceRobotId)) {
            throw new MqttContractViolationException(
                "NAVIGATION_RESULT robotId does not match wake-word scenario robot");
        }
        ...
        log.info("Wake-word call movement finished: scenarioId={}, status={}, resultCode={}, "
                + "reasonCode={}", scenarioId, scenario.getFinalStatus(), resultCode, reasonCode);
    }
